...
class Service(
    ...
    def _catalog(self, tx, user_id=None):
        ...
        unavailable = {
            d['id']
            for d in diets
            if not d.get('is_demo', False)
            and not (d.get('self_declared') is True and d.get('medical') is False)
            and (
                not d.get('evidence_references')
                or not set(d['evidence_references']) <= current_evidence
            )
        }
