...
class FeatureApi extends support.TestApi {
  ...
  Future<Json> request(String method, String path, {Json? body, String? operationKey, bool allowCache = true}) async {
    if (path == '/wellbeing') {
      ...
      if (method == 'POST') { completed = body!['completed'] as bool; version++; }
      return {'version':version,'week_start':'2026-09-07','goals':[{'id':'waste_less','target':5,'completed_days':completed ? 1 : 0,'done_today':completed}]};
    }
...
void main() {
  testWidgets('habit check-in and undo use the latest version and reload progress', (tester) async {
    ...
    expect(updates.map((c) => c['body']['expected_version']).toList(),[1,2]);
    expect(updates.every((c) => c['key'] != null),isTrue);
    expect(tester.takeException(),isNull);
  });
  testWidgets('custom food rejects an empty name before uploading or saving', (tester) async {
    final api = FeatureApi();
  ...
  testWidgets('scan review requires explicit confirmation and keeps remaining item edits', (tester) async {
    final api = FeatureApi();
    final job = <String,dynamic>{'id':'scan','result':{'items':[
      {'food_id':'tomato','name':'First item','quantity':'100','unit':'g','confidence':0.8},
      {'food_id':'tomato','name':'Second item','quantity':'250','unit':'g','confidence':0.7},
    ]}};
    await tester.pumpWidget(support.harness(DetectionReviewPage(job:job),api));
    await tester.pumpAndSettle();
