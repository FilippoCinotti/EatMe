...
class DietHealthPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final intolerances = List<String>.from(
      settings['intolerances'] as List? ?? [],
    );
    return Scaffold(
      ...
      body: PageBody(
        children: [
              ...
              children: [
                ...
                for (final section in [
                  ...
                  if (section.$2.isEmpty)
                    Text(context.t('none_recorded'))
                  else
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: [
                        for (final value in section.$2)
                          StatusBadge(
                            label: context.t(
                              section.$1 == 'allergies'
                                  ? 'allergen_$value'
          ...
          SettingsGroup(
            title: context.t('diet'),
            children: [
              if (assignments.isEmpty)
                Padding(
                  padding: const EdgeInsets.symmetric(vertical: 16),
                  child: Text(context.t('no_diet')),
                ),
              for (final assignment in assignments)
                Builder(
                  builder: (context) {
                    final diet = state.diets
                        .where((d) => d.id == assignment['diet_id'])
                        .firstOrNull;
                    return SettingRow(
                      title: diet == null
                          ? context.t('food_unavailable')
          ...
          if (settings['primary_goal'] is String)
            SettingsGroup(
