...
class OfflineStore {
  ...
  Future<void> cache(String path, Json value) async {
    final content = jsonEncode({'saved_at': DateTime.now().toUtc().toIso8601String(), 'data': value});
  ...
  Future<Json?> read(String path) async {
    ...
    try {
      ...
      return {...Map<String, dynamic>.from(value['data'] as Map), '_offline': true, '_saved_at': value['saved_at']};
    } on FormatException { return null; }
  }
  Future<List<Json>> pending() async {
    final raw = await storage.read(key: '${prefix}outbox');
    if (raw == null) return [];
    return (jsonDecode(raw) as List).map((v) => Map<String, dynamic>.from(v as Map)).toList();
  }
  Future<void> savePending(List<Json> value) => storage.write(key: '${prefix}outbox', value: jsonEncode(value));
  Future<void> clear() async {
    for (final key in (await storage.readAll()).keys.where((key) => key.startsWith(prefix))) {
