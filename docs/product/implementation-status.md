...
The implemented application compiles into an Android debug APK and an iOS simulator application. API, database isolation, studio and Flutter widget checks have passed. The real API Android emulator workflow is the remaining automated application verification gate; read `verification.md` for its latest result and the exact tested commit. Development fixtures are not production data and are not evidence of recognition accuracy or clinical validity.
