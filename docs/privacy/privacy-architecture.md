...
Exports include profile, inventory, events, cooking, consent, plans, shopping, preferences, feedback, private recipes, processing records, media metadata, hosted Dinner events and explicitly enabled Meal Memory records. Account deletion verifies ownership prerequisites, deletes the identity and removes associated personal records and media. Shared contributions are handled according to the data model and documented release policy. Store subscriptions require separate cancellation.

Dinner responses are purpose-limited to event planning. Temporary guest participants and responses are removed after the configured short post-event retention window. “Remember my preferences” is off by default and creates a household-scoped saved guest only after explicit guest opt-in; deleting the response removes the saved copy created by that opt-in. Meal Memory stores the event name, menu and serving count, not guest health details.
